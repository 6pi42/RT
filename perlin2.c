...
/*  Updated: 2016/06/30 12:20:55 by apaget      ###  ########.fr    */
/*                                      */
/* ************************************************************************** */
...
float	get_sommet(t_perlin *perl, int x, int y)
{
	...
	int		rand_vector;
	float	unit;
	unit = 0.7f;//1.0f/sqrt(2);
	float	gradient[8][2] = {{unit, unit}, {-unit, unit}, {unit, -unit}, 
...
int		calc_pt_color(t_perlin *perl, int x, int y, float res)
{
	...
	return ((lx1 + coef_lissage_y * (lx2 - lx1)) * 500);
}
...
int		*get_perl_tex(int height, int width, float res)
{
	...
	while (x < width)
	{
		...
		while (y < height)
		{
			...
			tex[i] = tex[i] < 0 ? -tex[i] : tex[i];
			tex[i] = fmin(tex[i], 255.0f);
			i++;
			y++;
		}
		x++;
