...
/*   Updated: 2016/07/18 15:32:44 by amathias         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	connect_check_button(GtkWidget *widget, t_map *map)
{
	if (ft_strsearch("texture", (char*)gtk_widget_get_name(widget)) != -1)
		gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(widget),
			map->config.texture);
	else if (ft_strsearch("refraction",
		(char*)gtk_widget_get_name(widget)) != -1)
		gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(widget),
			map->config.refraction);
	else if (ft_strsearch("reflection",
		(char*)gtk_widget_get_name(widget)) != -1)
		gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(widget),
			map->config.reflection);
	else if (ft_strsearch("filtre noir et blanc",
		(char*)gtk_widget_get_name(widget)) != -1)
		gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(widget),
			map->config.filtre_n);
	else if (ft_strsearch("filtre sepia",
		(char*)gtk_widget_get_name(widget)) != -1)
		gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(widget),
			map->config.filtre_s);
	connect_check_button2(widget, map);
}
...
void	check_bar2(GtkWidget *widget, t_map *map)
{
	if (ft_strsearch("transparence",
		(char*)gtk_widget_get_name(widget)) != -1)
		map->config.transparence = gtk_toggle_button_get_active(
			GTK_TOGGLE_BUTTON(widget));
	else if (ft_strsearch("ombre", (char*)gtk_widget_get_name(widget)) != -1)
		map->config.ombre = gtk_toggle_button_get_active(
		GTK_TOGGLE_BUTTON(widget));
	else if (ft_strsearch("print_neg",
		(char*)gtk_widget_get_name(widget)) != -1)
	{
		map->config.print_neg = gtk_toggle_button_get_active(
			GTK_TOGGLE_BUTTON(widget));
		clSetKernelArg(map->env.kernel, 4, sizeof(cl_int),
...
{
	if (ft_strsearch("texture", (char*)gtk_widget_get_name(widget)) != -1)
		map->config.texture = gtk_toggle_button_get_active(
			GTK_TOGGLE_BUTTON(widget));
	else if (ft_strsearch("refraction",
		(char*)gtk_widget_get_name(widget)) != -1)
		map->config.refraction = gtk_toggle_button_get_active(
			GTK_TOGGLE_BUTTON(widget));
	else if (ft_strsearch("reflection",
		(char*)gtk_widget_get_name(widget)) != -1)
		map->config.reflection = gtk_toggle_button_get_active(
			GTK_TOGGLE_BUTTON(widget));
	else if (ft_strsearch("filtre noir et blanc",
		(char*)gtk_widget_get_name(widget)) != -1)
		map->config.filtre_n = gtk_toggle_button_get_active(
			GTK_TOGGLE_BUTTON(widget));
	else if (ft_strsearch("filtre sepia",
		(char*)gtk_widget_get_name(widget)) != -1)
		map->config.filtre_s = gtk_toggle_button_get_active(
			GTK_TOGGLE_BUTTON(widget));
	else
		check_bar2(widget, map);
	draw(map);
}
...
void	manip_conf(t_map *map, GtkWidget *interface)
{
	...
	gtk_box_pack_start(GTK_BOX(global_box), GTK_WIDGET(box), FALSE, TRUE, 10);
	gtk_box_pack_start(GTK_BOX(interface), GTK_WIDGET(global_box),
		FALSE, TRUE, 10);
}
