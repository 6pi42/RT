...
/*   Updated: 2016/07/18 14:29:25 by apaget           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	add_coef_scroll_bar(GtkWidget *interface, char *label_str, t_map *map)
{
	...
	if (ft_strsearch(label_str, "Ka :") != -1)
		scroll = gtk_scale_new_with_range(GTK_ORIENTATION_HORIZONTAL, 0.2, 1,
				0.05);
	else
		scroll = gtk_scale_new_with_range(GTK_ORIENTATION_HORIZONTAL, 0, 1,
				0.05);
	gtk_box_pack_start(GTK_BOX(box), GTK_WIDGET(label), FALSE, TRUE, 10);
